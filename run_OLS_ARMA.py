...
def main():
    ...
    if "sample" in stock_path:
        ...
        print("The data is proprietary and could therefore not be provided in full")
        print("For meaningful results, please add more data in the same format")
        print("\n\n")
...
def OLS_and_ARMA(index, city):
    ...
    data['VeryCold'] = np.where(data['Mean Temperature Actual'] \
        <= np.percentile(data['Mean Temperature Actual'], q=25), 1, 0)
    data['HeavyRain'] = np.where(data['Precipitation Actual'] \
        >= np.percentile(data['Precipitation Actual'], q=75), 1, 0)
    ...
    data['DayDiff'] = data['High Temperature Actual'] - data['Low Temperature Actual']
    ...
    data_cut = data[1:]
    X = data_cut[['LagReturn','VeryCold','HeavyRain','ColdRain','Monday','Winter','Summer']]
    ...
    X = data_cut[['LagReturn', 'DayDiff', 'HeavyRain', 'Monday', 'Winter', 'Summer']]
...
def OLS_global(index):        
    print("\n==========================================================\n")
    print("\nRunning regression on index {}, using binary variables for global weather conditions...".format(index))
    stock_index, weather = ld.load_data_OLS(stock_path, weather_path, index)
